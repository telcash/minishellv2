...
/*   Updated: 2025/03/30 19:30:17 by carlossalaz      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *append_last_exit_code(char *expanded_data, int *i)
{
    expanded_data = append_to_expanded_data(expanded_data, "999");
    *i += 2;
    return expanded_data;
}

static char *append_env_var_value(char *expanded_data, char *data, int *i, t_shell *shell)
{
    int end;
    char *tmp;
    char *new_expanded_data;
    ...
    new_expanded_data = append_to_expanded_data(expanded_data, get_env_var_value(shell, tmp));
    free(tmp);
    *i = end;
    return new_expanded_data;
}
...
char *expand_variable(char *data, t_shell *shell)
{
    ...
    while (data[i])
    {
        if (data[i] != '$')
            ...
            expanded_data = append_last_exit_code(expanded_data, &i);
        else
            expanded_data = append_env_var_value(expanded_data, data, &i, shell);
    }
    return expanded_data;
}
